        ...
        for csv_file in os.listdir(samples_path):
            ...
            try:
                ...
                for frame_id in sorted(df['Frame'].unique()):
                    ...
                    frame_vectors.append(frame_values)

                # Pad frames to ensure same size (max length = 51 * 3 = 153)
                max_features_per_frame = 153
                padded_frames = []
                for vec in frame_vectors:
                    if len(vec) < max_features_per_frame:
                        padded = np.pad(vec, (0, max_features_per_frame - len(vec)))
                    else:
                        padded = vec[:max_features_per_frame]
                    padded_frames.append(padded)

                sample_array = np.stack(padded_frames)  # shape: (20, 153)
...
# Load training data
train_base_dir = "../../../train-hand-body-face"
data, labels, class_mapping = load_data(train_base_dir)

# NEWWWWWWWWWWWWWW REMOOOOVEEE classes
...
labels = np.array([label_remap[label] for label in labels])
# DEBUG!!!!
#from collections import Counter
...
# ✅ Update class_mapping to match remapped labels
inv_class_mapping = {v: k for k, v in class_mapping.items()}
remapped_class_mapping = {inv_class_mapping[old]: new for old, new in label_remap.items()}

# ✅ Set the correct label order for encoder
label_encoder.classes_ = np.array([key for key, val in sorted(remapped_class_mapping.items(), key=lambda item: item[1])])

# ✅ Save encoder
joblib.dump(label_encoder, 'label_encoder.pkl')

### NEW REMAP
joblib.dump(label_remap, "label_remap.pkl")

# Data scaling
num_features = data.shape[2]

scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train.reshape(-1, 20 * num_features))
X_train_scaled = X_train_scaled.reshape(-1, 20, num_features)

X_val_scaled = scaler.transform(X_val.reshape(-1, 20 * num_features))
X_val_scaled = X_val_scaled.reshape(-1, 20, num_features)
